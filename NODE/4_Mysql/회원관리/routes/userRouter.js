...
router.post("/join", (req, res) => {
  ...
  // ⭐️ 포스트로 넘긴 데이터는 req안에 body로 존재한다!
  //console.log("넘어온 데이터 : ", req.body);
  let { id, pw, nick } = req.body;

  // 2. DB와 연결해서 데이터 처리
  // 2-1. 처리할 sql 문장
  // 2-2. 입력할 데이터가 필요한 경우 -> 값을 넣어주기!
  // 2-3. 처리할 콜백함수
...
router.post("/login", (req, res) => {
  ...
  // 2. conn 쿼리문 실행 (sql구문, [데이터], (err, rows))
  // 3. id와 pw가 일치하면 콘솔창에 로그인 성공!/ 일치하지 않으면 로그인 실패!
...
router.post("/update", (req, res) => {
  ...
  // 쿼리문 생성
  // update문을 활용해서 조건을 부여할 때는 affectedRows를 활용할 것!
