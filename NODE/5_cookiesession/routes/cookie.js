...
router.get("/setCookie", (req, res) => {
  ...
  // 쿠키 만료 설정 - 하지 않으면 계속 브라우저에 존재 -> 성능 저하
  // 방법1 - maxAge -> ms단위로 설정
  res.cookie("name", "baek", { maxAge: 5000 });

  // 방법2 - expires -> 현재 날짜 + 초 * 분 * 시 * 일
  // - 쿠키의 수명만료를 날짜로 설정
...
router.get("/getCookie", (req, res) => {
  // 쿠키를 불러올 때는 req를 사용한다.
  console.log("쿠키 데이터", req.cookies.nick);
  console.log("쿠키 데이터 생명추가", req.cookies.name);
  console.log("쿠키 데이터 생명추가2", req.cookies.age);
  res.redirect("/");
});
